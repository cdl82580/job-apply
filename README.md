...
- **ATS resume** — plain single-column DOCX, no tables or text boxes, parser-safe; content is parsed verbatim from the tailored styled resume's XML (`scripts/gen_ats_from_styled.py`) rather than generated separately, so the two documents can't drift out of sync
